...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in train_pbar:
        ...
        # 生成掩码（标签≠10的位置为1，否则为0）
        mask = (target != 10).float()  # 形状：(batch_size, 5)

        # 计算各位置损失（每个损失形状：(batch_size,)）
        loss0 = criterion(c0, target[:, 0].long()) * mask[:, 0]
        loss1 = criterion(c1, target[:, 1].long()) * mask[:, 1]
        loss2 = criterion(c2, target[:, 2].long()) * mask[:, 2]
        loss3 = criterion(c3, target[:, 3].long()) * mask[:, 3]
        loss4 = criterion(c4, target[:, 4].long()) * mask[:, 4]

        # 总损失：先按样本求平均（避免除零），再对批量求平均（得到标量）
        per_sample_loss = (loss0 + loss1 + loss2 + loss3 + loss4) / (mask.sum(dim=1, keepdim=True) + 1e-8).clamp(
            min=1e-8)
        loss = per_sample_loss.mean()  # 关键修改：对批量内的样本损失求平均，得到标量
...
for epoch in range(20):
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer,
        mode='min',  # 监控指标是验证损失（越小越好）
        patience=3,  # 验证损失连续3轮不下降则调整
        factor=0.5,  # 学习率乘以0.5（衰减一半）
        verbose=True  # 打印调整日志
    )
